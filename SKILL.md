...
3. **A brand-new Apps Script project needs one manual, interactive authorization before its web app will work — `clasp deploy` alone cannot grant this.** Right after `clasp create` + `clasp push` + `clasp deploy`, the `/exec` endpoint returns a Google Drive-style "Access denied — You need access" page, even though the manifest correctly says `access: ANYONE_ANONYMOUS` and even for the deploying owner. This is because the script has never had its own requested scopes (Sheets access) interactively consented to — clasp's login OAuth is a *different* grant from the *script's own* runtime authorization. Fix: open the Apps Script editor (`clasp open` or the URL from `clasp create`'s output), select any function (e.g. `doGet`) in the toolbar, click Run once, and click through the "Review permissions" → "Advanced" → "Go to (unsafe)" → "Allow" prompts. The run itself will error (no valid request object), which is expected and fine — only the authorization matters. After that one-time click-through, the deployed URL starts working immediately. This has to be done by whoever owns the script; it cannot be scripted.
4. **Serialize per-row save requests, or fast consecutive actions can silently lose data.** Casting a GSB vote, then immediately setting quality status, then immediately ticking a reason, each independently calls `submitRow()` → a `fetch()` POST. Every payload is built synchronously so it's never stale *at construction time*, but the requests race over the network — if a later (more complete) request's response arrives before an earlier one's, the upsert on the Sheet ends up with the earlier, incomplete snapshot as the final row content. `submitRow()` queues one in-flight request per row (`pendingSubmits[rowIndex]`, a promise chain) so they always land server-side in call order. Don't remove this queue when editing `submitRow()` — a quick way to reintroduce silent data loss.
5. **A numeric-looking evaluator name breaks the upsert unless every comparison goes through `String()`.** If someone types e.g. "720" as their name, Google Sheets stores that cell as an actual Number, not text. `Code.gs`'s upsert lookup (`submitVote_`, `getMyVotes_`, `submitCompletion_`) compares the evaluator name read back from the sheet against the string the client sent; a bare `===` between a Number and a String is always false, so the "find the existing row" search never matches and every subsequent save from that evaluator *appends a new row* instead of updating — their answers end up fragmented across several incomplete duplicate rows for the same `row_index` instead of one final row. All three lookups must wrap both sides in `String(...)` (already done in `assets/Code.gs` — verify this wasn't lost if you hand-edit it). This is easy to miss in testing because most test evaluator names are words, not numbers.
